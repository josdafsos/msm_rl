...
class SimulinkPlant:
    ...
    @staticmethod
    def set_debug_mode(b_mode):
        SimulinkPlant.common_debug_mode = b_mode
    ...
    def __init__(self, modelName='MSM_strain_wave_actuator',
                 path='auto',  # path='C:\\Users\\h19343\\files\\PhD\\MSM actuators simualtion\\Simscape model',
                 scaler=None,
                 debug_mode=None,
                 model_index=None,
                 controller="nn"):
        ...
        if path == 'auto':
            path = utils.get_matlab_model_path()
        self.path = path
        ...
        if debug_mode is not None:
            ...
            self.debug_mode = SimulinkPlant.common_debug_mode
        ...
        self.reset_plant()
        if self.debug_mode:
            print('DEBUG: Plant initialization finished')
    ...
    def set_desired_velocity(self, type="random"):
        ...
        elif type == "random_low":
            self.velocity_setpoint = -random.random() * 0.0045 - 0.001
        elif type == "random_high":
            self.velocity_setpoint = -random.random() * 0.0045 - 0.0055
        else:
    ...
    def init_simulation_params(self):
        ...
        self.eng.eval("SIMULATION_TIME = {:F};".format(0.005), nargout=0)  # format(0.02)  format(0.03)
    ...
    def reset_plant(self):
        ...
        if self.debug_mode:
            print("DEBUG: reseting system. ", datetime.now().strftime("%d/%model/%Y %H:%M:%S:%MS"))
        ...
        self.data_dict = {}
        if self.debug_mode:
            print("DEBUG:system was reseted. ", datetime.now().strftime("%d/%model/%Y %H:%M:%S:%MS"))
    ...
    def step(self, action):  # TODO
        ...
        if self.steps_cnt > 0:  # not enough data on the 0th iteration
            self.eng.eval("external_control_value = {:F};".format(float(action)), nargout=0)

        self.eng.set_param(self.modelName, 'SimulationCommand', 'update', 'SimulationCommand', 'continue', nargout=0)
        while self.eng.get_param(self.modelName, 'SimulationStatus') != ('paused' or 'stopped' or 'terminating'):

            time.sleep(0.05)
            # for some reason the unified condition does not work
            if self.eng.get_param(self.modelName, 'SimulationStatus') == 'paused' \
              or self.eng.get_param(self.modelName, 'SimulationStatus') == 'stopped'\
              or self.eng.get_param(self.modelName, 'SimulationStatus') == 'terminating':
                break
            # if self.eng.get_param(self.modelName, 'SimulationStatus') == 'paused':
            #     print('exiting while loop')
            #     break

        sim_status = self.eng.get_param(self.modelName, 'SimulationStatus')


        #self.get_sim_data()
        observation = np.float32(np.array([0, 0, 0, 0]))  # TODO
        reward = 0  # TODO
        ...
        terminated = sim_status == ('stopped' or 'terminating')
        if self.debug_mode and terminated:
            print('DEBUG: simulation was terminated, terminated value: ', terminated)
    ...
    def run_simulation(self, pygad_solution=None):
        ...
        # find the index of transient process finish (based on time) to compute complex error function
        transition_idx = bisect.bisect_right(self.data_dict["time"], 0.0015)

        unequal_sign_cnt = np.sum((np.sign(self.data_dict["velocity"][transition_idx:]) != np.sign(self.data_dict["desired_velocity"][transition_idx:])).astype(int))  # penalty for keeping velocity around zero
        mse_transition = -np.mean(np.square(self.data_dict["velocity"][:transition_idx] - self.data_dict["desired_velocity"][:transition_idx]))
        mse_steady = -np.mean(np.square(self.data_dict["velocity"][transition_idx:] - self.data_dict["desired_velocity"][transition_idx:]))
        fitness = mse_transition*0.5 + mse_steady * (unequal_sign_cnt + 1)
        #fitness = fitness[-1]
...
class SimPool:
    ...
    def initialize(debug_mode=None):
        ...
        instance = SimulinkPlant(debug_mode=debug_mode, model_index=idx)
...
"""
model = utils.get_nn_model(folder="pygad_networks_py_12", controller_name="LSTM_Relu_min_loss_0.0179.keras")

#model_path = os.path.join('~','ik_files', 'msm_matlab_model')
t0 = time.time()


# plant = SimulinkPlant(debug_mode=False, controller="pid")
# plant.reset_plant()
...
plant = SimulinkPlant(debug_mode=False, controller="nn")
plant.reset_plant()
t0 = time.time()
plant.set_nn_model(model)
fitness = plant.run_simulation()
